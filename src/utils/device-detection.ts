 ...
 */
export function hasIntegratedGPU(gl?: WebGLRenderingContext | null): boolean {
    if (!gl) {
    ...
    try {
        ...
        if (debugInfo) {
            ...
            if (typeof renderer === 'string') {
                ...
                const integratedGPUPattern = /Intel|AMD.*Integrated|Mali|Adreno|PowerVR|VideoCore/i;
 ...
 */
export function getHardwareInfo(gl?: WebGLRenderingContext | null): HardwareInfo {
    return {
 ...
 */
export function getMobileRayStepCap(requestedSteps: number, isMobile: boolean): number {
    if (isMobile) {
