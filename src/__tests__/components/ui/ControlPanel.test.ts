...
describe('ControlPanel UI Controls - Property-Based Tests', () => {
    ...
    it('Property 25: Parameter bounds indication', () => {
        fc.assert(
            fc.property(
                fc.record({
                    min: fc.float({ min: Math.fround(0), max: Math.fround(10) }).filter(x => !isNaN(x) && isFinite(x)),
                    max: fc.float({ min: Math.fround(10), max: Math.fround(100) }).filter(x => !isNaN(x) && isFinite(x)),
                    step: fc.float({ min: Math.fround(0.01), max: Math.fround(1) }).filter(x => !isNaN(x) && isFinite(x) && x > 0)
                }),
                ({ min, max, step }) => {
    ...
    it('Property 26: Pause freezes time', () => {
        fc.assert(
            fc.property(
                fc.record({
                    initialTime: fc.float({ min: Math.fround(0), max: Math.fround(10000) }).filter(x => !isNaN(x) && isFinite(x)),
                    deltaTime: fc.float({ min: Math.fround(0.001), max: Math.fround(0.1) }).filter(x => !isNaN(x) && isFinite(x) && x > 0),
                    paused: fc.boolean()
                }),
                ({ initialTime, deltaTime, paused }) => {
                    // Simulate time update logic
                    const updateTime = (currentTime: number, dt: number, isPaused: boolean): number => {
    ...
    it('Property 27: Mass change updates radii', () => {
        fc.assert(
            fc.property(
                fc.record({
                    mass: fc.float({ min: Math.fround(0.1), max: Math.fround(3.0) }).filter(x => !isNaN(x) && isFinite(x) && x > 0),
                    spin: fc.float({ min: Math.fround(-1.0), max: Math.fround(1.0) }).filter(x => !isNaN(x) && isFinite(x))
                }),
                ({ mass, spin }) => {
                    ...
                    // Event horizon should be proportional to mass
                    const expectedEventHorizonScale = mass * 0.5; // Approximate Schwarzschild radius
                    expect(eventHorizon).toBeGreaterThan(0);
                    expect(eventHorizon).toBeLessThanOrEqual(expectedEventHorizonScale * 2);
