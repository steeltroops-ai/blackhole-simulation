...
describe('Feature Toggles - Property-Based Tests', () => {
    ...
    describe('Property 1: Quality level to ray steps mapping', () => {
        it('should map quality levels to correct ray steps', () => {
            fc.assert(
                fc.property(
                    fc.constantFrom<RayTracingQuality>('off', 'low', 'medium', 'high', 'ultra'),
                    (quality) => {
        ...
        it('should have monotonically increasing steps for increasing quality', () => {
            const qualities: RayTracingQuality[] = ['off', 'low', 'medium', 'high', 'ultra'];
            ...
            fc.assert(
                fc.property(
                    ...
                    (presetName) => {
                        ...
                        expect(preset.rayTracingQuality).toBe('low');
    ...
    describe('Property 4: Balanced preset configuration', () => {
        it('should enable disk and lensing, disable bloom and Doppler, set ray tracing to medium', () => {
            fc.assert(
                fc.property(
                    fc.constant('balanced' as PresetName),
                    (presetName) => {
                        const preset = getPreset(presetName);
    ...
    describe('Property 5: High Quality preset configuration', () => {
        it('should enable all features except bloom, set ray tracing to high', () => {
            fc.assert(
                fc.property(
                    fc.constant('high-quality' as PresetName),
                    (presetName) => {
                        const preset = getPreset(presetName);
    ...
    describe('Property 7: Preset application synchronicity', () => {
        it('should return a complete feature toggles object in a single operation', () => {
            fc.assert(
                fc.property(
                    fc.constantFrom<PresetName>('maximum-performance', 'balanced', 'high-quality', 'ultra-quality'),
                    (presetName) => {
                        const preset = getPreset(presetName);
            ...
            fc.assert(
                fc.property(
                    ...
                    (features) => {
                        ...
                        const validPresets: PresetName[] = ['maximum-performance', 'balanced', 'high-quality', 'ultra-quality', 'custom'];
