...
describe('Property 8: Rolling average FPS calculation', () => {
    test('rolling average FPS equals 1000 divided by average frame time', () => {
        fc.assert(
            fc.property(
                ...
                fc.array(fc.double({ min: 10, max: 100, noNaN: true }), { minLength: 1, maxLength: 60 }),
                (frameTimes) => {
    ...
    test('rolling average uses only last 60 frames', () => {
        fc.assert(
            fc.property(
                ...
                fc.array(fc.double({ min: 10, max: 100, noNaN: true }), { minLength: 61, maxLength: 120 }),
                (frameTimes) => {
                    // Create fresh monitor for each test
                    const monitor = new PerformanceMonitor();
                    ...
                    const avgFrameTime = last60Frames.reduce((sum, time) => sum + time, 0) / last60Frames.length;
                    const expectedFPS = Math.round(1000 / avgFrameTime);
    ...
    test('rolling average FPS is always positive for positive frame times', () => {
        fc.assert(
            fc.property(
                fc.array(fc.double({ min: 1, max: 100, noNaN: true }), { minLength: 1, maxLength: 60 }),
                (frameTimes) => {
                    // Create fresh monitor for each test
                    const monitor = new PerformanceMonitor();
        ...
        fc.assert(
            fc.property(
                ...
                (lowFrameTime, highFrameTime) => {
                    ...
                    expect(metrics1?.rollingAverageFPS).toBeGreaterThanOrEqual(metrics2?.rollingAverageFPS || 0);
                }
            ),
            { numRuns: 100 }
        );
        ...
        fc.assert(
            fc.property(
                ...
                (baselineFrameTime, spikeFrameTime) => {
                    ...
                    expect(metricsWithSpike.rollingAverageFPS).toBeGreaterThan(instantaneousFPS);
                }
            ),
            { numRuns: 100 }
        );
        ...
        fc.assert(
            fc.property(
                ...
                (frameTimes) => {
                    ...
                    expect(lastMetrics?.rollingAverageFPS).toBe(Math.round(lastMetrics?.rollingAverageFPS || 0));
                }
            ),
            { numRuns: 100 }
        );
...
describe('PerformanceMonitor', () => {
    ...
    test('setRenderResolution clamps to valid range', () => {
        monitor.setRenderResolution(0.3); // Below minimum
        let metrics = monitor.updateMetrics(16.67);
        expect(metrics.renderResolution).toBe(0.5);

        monitor.setRenderResolution(1.5); // Above maximum
        metrics = monitor.updateMetrics(16.67);
        expect(metrics.renderResolution).toBe(1.0);
    });
    ...
    test('getFrameTimeBudgetUsage returns percentage', () => {
        monitor.updateMetrics(13.3); // Exactly at budget
