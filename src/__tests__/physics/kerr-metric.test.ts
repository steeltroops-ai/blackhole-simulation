        ...
        fc.assert(
            fc.property(
                ...
                (mass, spin) => {
                    ...
                    const rs = mass * 1.0; // Schwarzschild radius = 2GM/c² = M in simulation units
        ...
        fc.assert(
            fc.property(
                ...
                (mass) => {
                    ...
                    const rs = mass * 1.0; // Schwarzschild radius = M in simulation units
...
describe('Property 2: Photon sphere ordering', () => {
    test('photon sphere is always between event horizon and ISCO for moderate spin', () => {
        fc.assert(
            fc.property(
                fc.double({ min: 0.1, max: 10.0, noNaN: true }), // mass
                fc.double({ min: 0.0, max: 0.90, noNaN: true }), // spin (exclude extreme values near 1.0)
                (mass, spin) => {
                    const rh = calculateEventHorizon(mass, spin);
    ...
    test('photon sphere is approximately 1.5x Schwarzschild radius for zero spin', () => {
        fc.assert(
            fc.property(
                fc.double({ min: 0.1, max: 10.0, noNaN: true }), // mass
                (mass) => {
                    const rs = mass * 1.0; // Schwarzschild radius = M
        ...
        fc.assert(
            fc.property(
                ...
                (mass) => {
                    ...
                    const expected = mass * 0.5 * 6.0; // 6M in geometric units
        ...
        fc.assert(
            fc.property(
                ...
                (mass, radiusMultiplier) => {
                    const rs = mass * 1.0; // Schwarzschild radius
