...
export class PerformanceValidator {
        ...
        return new Promise((resolve) => {
            ...
            const measureFrame = () => {
                ...
                if (onProgress) {
                    ...
                    const progress = Math.min(elapsed / this.MEASUREMENT_DURATION_MS, 1.0);
                    onProgress(progress);
                }

                // Check if measurement complete
                if (currentTime >= measurementEnd) {
                    resolve(this.calculateMeasurement(
                        configName,
                        features,
                        fpsReadings,
                        frameTimeReadings,
    ...
    private calculateMeasurement(
        ...
        const averageFrameTimeMs = frameTimeReadings.reduce((a, b) => a + b, 0) / frameTimeReadings.length;
    ...
    async measureBaseline(
        ...
        return this.measureConfiguration(baselineFeatures, 'Baseline (All Features Off)', onProgress);
    }
    ...
    async measureFeatureCosts(
        ...
        costs.push(this.calculateFeatureCost(
            'Gravitational Lensing',
            baselineMeasurement,
            lensingMeasurement
        ));
    ...
    private calculateFeatureCost(
        ...
        const frameTimeImpactMs = featureEnabled.averageFrameTimeMs - baseline.averageFrameTimeMs;
    ...
    async generateValidationReport(
        ...
        const baselineMeasurement = await this.measureBaseline(
            (p) => onProgress?.('Measuring baseline performance', p)
        );
        ...
        const meetsTargets = {
            ...
            mobile60FPS: deviceInfo.isMobile ? baselineMeasurement.averageFPS >= 60 : true, // Requirement 1.3
            desktop120FPS: !deviceInfo.isMobile ? baselineMeasurement.averageFPS >= 120 : true, // Requirement 1.4
        };
    ...
    private generateRecommendations(
        baseline: PerformanceMeasurement,
        costs: FeatureCost[],
        targets: { baseline75FPS: boolean; mobile60FPS: boolean; desktop120FPS: boolean },
        isMobile: boolean
    ): string[] {
