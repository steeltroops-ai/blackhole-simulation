...
export class WebGPURenderer {
    ...
    async initPipelines(swapChainFormat: GPUTextureFormat, maxSteps: number = 150) {
        if (!this.device) return;
    ...
    public render(camera: CameraUniforms, physics: PhysicsParams) {
        if (!this.device || !this.computePipeline || !this.renderPipeline || !this.context || !this.computeTexture) return;
