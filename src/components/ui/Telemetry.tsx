...
export const Telemetry = ({
  ...
  const timeDilation = calculateTimeDilation(
  ...
  return (
    <div className="flex flex-col items-end gap-4">
      <div className="flex gap-6 text-right">
        <div className="group">
          <p className="text-[9px] text-gray-500 uppercase tracking-wider mb-1 group-hover:text-blue-400 transition-colors">
            Event Horizon
          </p>
          <p className="font-mono text-base md:text-xl">
            {eventHorizonRadius.toFixed(2)}{" "}
            <span className="text-[10px] text-gray-600">Rs</span>
          </p>
        </div>
        <div className="group">
          <p className="text-[9px] text-gray-500 uppercase tracking-wider mb-1 group-hover:text-orange-400 transition-colors">
            Redshift
          </p>
          <p className="font-mono text-base md:text-xl">
            z={redshift.toFixed(2)}
          </p>
        </div>
        <div className="group">
          <p className="text-[9px] text-gray-500 uppercase tracking-wider mb-1 group-hover:text-purple-400 transition-colors">
            Time Dilation
          </p>
          <p className="font-mono text-base md:text-xl">
      ...
      {metrics && (
        <div className="flex flex-col gap-2 mt-2 pt-2 border-t border-white/10">
          <div className="flex gap-4 text-right">
            ...
            >
              <p className="text-[9px] text-gray-500 uppercase tracking-wider mb-1 group-hover:text-green-400 transition-colors">
                FPS
              </p>
              <p
                className={`font-mono text-sm ${getFPSColor(metrics.currentFPS)}`}
              >
                {metrics.currentFPS}
              </p>
            </div>
            ...
            <div className="group">
              ...
              <p
                className={`font-mono text-sm ${getFPSColor(metrics.rollingAverageFPS)}`}
              >
            ...
            <div className="group">
              <p className="text-[9px] text-gray-500 uppercase tracking-wider mb-1 group-hover:text-cyan-400 transition-colors">
                Quality
              </p>
              <p className="font-mono text-sm text-cyan-400 uppercase">
                {metrics.quality}
              </p>
            </div>
          </div>
            ...
            <div className="absolute top-full right-0 mt-2 bg-black/90 backdrop-blur-md border border-white/20 rounded-lg p-3 min-w-[200px] shadow-xl z-50 pointer-events-none">
              ...
              <div className="space-y-1.5 text-[11px]">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-mono ${getFPSColor(metrics.currentFPS)}`}
                  >
                    {metrics.currentFPS}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-mono ${getFPSColor(metrics.rollingAverageFPS)}`}
                  >
                    {metrics.rollingAverageFPS}
                  </span>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-mono ${budgetUsage > 100 ? "text-red-400" : budgetUsage > 80 ? "text-yellow-400" : "text-green-400"}`}
                  >
                    {Math.round(budgetUsage)}%
                  </span>
                </div>
                ...
                {(metrics as any).gpuMemoryUsageMB && (
                  <div className="flex justify-between">
                    <span className="text-gray-400">GPU Memory:</span>
                    <span className="font-mono text-purple-400">
                      {(metrics as any).gpuMemoryUsageMB.toFixed(0)}MB
                    </span>
                  </div>
                )}
              </div>
