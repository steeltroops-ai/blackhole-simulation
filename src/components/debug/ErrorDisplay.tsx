...
export const ErrorDisplay = ({
  ...
  const classifyError = (error: any) => {
    if (!error) return { code: "UNKNOWN", category: "UNKNOWN", message: "An unexpected error occurred" };
    ...
    if (error.status || error.code) {
      ...
      else if (typeof code === "number" && code >= 400) category = "HTTP_CLIENT";
    } else if (error.name) {
      category = "RUNTIME";
    ...
    if (message.includes("Cannot find module") || message.includes("manifest")) {
  ...
  const CATEGORY_MAP: Record<string, any> = {
    HTTP_CLIENT: { color: "text-orange-400", label: "Client Error", icon: AlertTriangle },
    HTTP_SERVER: { color: "text-red-500", label: "Server Error", icon: Server },
    RUNTIME: { color: "text-purple-400", label: "Runtime Error", icon: Activity },
    NETWORK: { color: "text-yellow-400", label: "Network Error", icon: WifiOff },
    AUTH: { color: "text-cyan-400", label: "Authentication Error", icon: ShieldAlert },
    DATA: { color: "text-pink-400", label: "Data Error", icon: FileJson },
    UNKNOWN: { color: "text-neutral-400", label: "Unknown Error", icon: AlertTriangle },
  };
      ...
      <div className="relative z-10 w-full max-w-2xl flex flex-col items-center text-center space-y-8 sm:space-y-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <span className="font-mono text-xs text-red-500 tracking-[0.3em] uppercase opacity-80">{code}</span>
            <div className="h-[1px] w-12 bg-gradient-to-l from-transparent to-red-500/50" />
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="space-y-4 max-w-md mx-auto">
          ...
          <div className="text-sm font-mono text-neutral-500 leading-relaxed uppercase tracking-wider">
            {category === "HTTP_SERVER" ? "Server-side manifest breach detected." : "Runtime exception occurred."}
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="flex flex-col sm:flex-row gap-4 w-full justify-center items-center">
          <button
            onClick={reset}
          ...
          >
            <RefreshCw className={`w-4 h-4 ${recoveryCount > 0 ? "text-red-400" : "text-white/70"} group-hover:rotate-180 transition-transform duration-500`} />
            <span className="text-[10px] uppercase font-bold tracking-widest">{recoveryCount > 0 ? "Force Reload" : "Retry System"}</span>
          </button>
          ...
          >
            ...
            <span className="text-[10px] uppercase font-bold tracking-widest">Return Home</span>
          </button>
        </motion.div>
        ...
        {error && (
          <div className="w-full max-w-xl">
            ...
            <AnimatePresence>
              {showDetails && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-6 overflow-hidden text-left">
                  <div className="rounded-2xl bg-neutral-900/80 border border-white/5 backdrop-blur-md overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
                      <div className="flex items-center gap-2">
                        <currentCategory.icon className={`w-3 h-3 ${currentCategory.color}`} />
                        <span className="text-[9px] font-mono text-neutral-400 uppercase tracking-wider">{error.name}</span>
                      </div>
                      <button onClick={handleCopy} className="text-[8px] uppercase tracking-widest text-neutral-500 hover:text-white flex items-center gap-1">
                        {copied ? <Check className="w-2 h-2 text-green-400" /> : <Copy className="w-2 h-2" />}
                    ...
                    <div className="p-4 max-h-[300px] overflow-y-auto font-mono text-[10px] leading-relaxed">
                      ...
                      <div className="mt-4 pt-4 border-t border-white/5 text-neutral-500 opacity-60 whitespace-pre-wrap">
                        {errorInfo?.componentStack || error.stack || "Stack trace unavailable."}
                      </div>
                    </div>
                  </div>
                </motion.div>
