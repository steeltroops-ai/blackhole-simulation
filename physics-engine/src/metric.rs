...
pub trait Metric {
    ...
    fn calculate_hamiltonian_derivatives(&self, r: f64, theta: f64, p: [f64; 4]) -> HamiltonianDerivatives;
...
impl Metric for KerrSchild {
    ...
    fn g_contravariant(&self, r: f64, theta: f64) -> [f64; 16] {
        ...
        let mut g = [0.0; 16];
        g[0]  = g_tt;   g[1]  = g_tr;   g[3]  = g_tph;
        g[4]  = g_tr;   g[5]  = g_rr;   g[7]  = g_rph;
        g[10] = g_thth;
        g[12] = g_tph;  g[13] = g_rph;  g[15] = g_phph;
    ...
    fn calculate_hamiltonian_derivatives(&self, r: f64, theta: f64, p: [f64; 4]) -> HamiltonianDerivatives {
        ...
        let dg_phph_dtheta = -(dsigma_dtheta * sin2 + sigma * 2.0 * sin_theta * cos_theta) / (sigma2 * sin2 * sin2);
