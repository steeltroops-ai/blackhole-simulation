...
pub fn calculate_derivatives(r: f64, theta: f64, p: [f64; 4], mass: f64, spin: f64) -> HamiltonianDerivatives {
    let a = spin * mass;
    let r2 = r * r;
    let a2 = a * a;
    ...
    let dg_tt_dr = - (du_dr * den_tphi - (sigma * (r2 + a2) + 2.0 * mass * r * a2 * sin2) * dv_dr) / (den_tphi * den_tphi);
    ...
    let dg_tt_dtheta = - (du_dtheta * den_tphi - (sigma * (r2 + a2) + 2.0 * mass * r * a2 * sin2) * dv_dtheta) / (den_tphi * den_tphi);
